...
alias cfg.nvim="fd --search-path ~/.config/nvim --type f | fzf  --preview 'exa -n --color=always {}' --multi --print0 | xargs -0 nvim -c 'cd ~/configs/nvim'"
...
alias yay.caches.drop='sudo paccache -rk3; yay -Sc --aur --noconfirm'
alias yay.servers.update='export TMPFILE="$(mktemp)"; \
    sudo true; \
